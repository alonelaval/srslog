...
type Formatter func(p Priority, hostname, tag, content string, appName string) string
...
func DefaultFormatter(p Priority, hostname, tag, content string, appName string) string {
...
func UnixFormatter(p Priority, hostname, tag, content string, appName string) string {
...
func RFC3164Formatter(p Priority, hostname, tag, content string, appName string) string {
	timestamp := time.Now().Format(time.Stamp)
	msg := fmt.Sprintf("<%d>%s %s %s[%d]: %s",
		p, timestamp, hostname, tag, os.Getpid(), content)
	return msg
...
func RFC5424Formatter(p Priority, hostname, tag, content string, appName string) string {
	timestamp := time.Now().Format(time.RFC3339)
	pid := os.Getpid()
	if appName == "" {
		appName = os.Args[0]
	...
	appName = truncateStartStr(appName, appNameMaxLength)
