...
func (n *localConn) writeString(framer Framer, formatter Formatter, p Priority, hostname, tag, msg string, appName string) error {
	if framer == nil {
	...
	_, err := n.conn.Write([]byte(framer(formatter(p, hostname, tag, msg, appName))))
	return err
}
